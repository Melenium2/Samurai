...
func TestTrackingDatabase_ShouldInsertInTransactionMetaToMetaTable_NoError(t *testing.T) {
	sqlDb, mock := MockDb()
	repo := db.New(getter_mock{}, db.NewMetaTracking(sqlDb), inserter_mock{}, inserter_mock{})
	...
	tx, _ := sqlDb.Begin()
	assert.NoError(t, repo.InsertTx(tx, context.Background(), info))
	assert.NoError(t, tx.Commit())
	assert.NoError(t, mock.ExpectationsWereMet())
}

func TestTrackingDatabase_ShouldInsertInTransactionAppToAppTable_NoError(t *testing.T) {
	sqlDb, mock := MockDb()
	repo := db.New(db.NewAppTracking(sqlDb), inserter_mock{}, inserter_mock{}, inserter_mock{})
...
func TestTrackingDatabase_ShouldInsertInTransactionCategoryToCategoryTable_NoError(t *testing.T) {
	sqlDb, mock := MockDb()
	repo := db.New(getter_mock{}, inserter_mock{}, inserter_mock{}, db.NewCategoryTracking(sqlDb))
	...
	tx, _ := sqlDb.Begin()
	assert.NoError(t, repo.InsertTx(tx, context.Background(), track))
	assert.NoError(t, tx.Commit())
	assert.NoError(t, mock.ExpectationsWereMet())
}

func TestTrackingDatabase_ShouldInsertKeywordInTransactionToKeywordsTable_NoError(t *testing.T) {
	sqlDb, mock := MockDb()
	repo := db.New(getter_mock{}, inserter_mock{}, db.NewKeywordsTracking(sqlDb), inserter_mock{})
...
func TestTrackingDatabaseInsertTx_ShouldInsertAppAndMetaInSingleTransaction_NoError(t *testing.T) {
	sqlDb, mock := MockDb()
	repo := db.New(db.NewAppTracking(sqlDb), db.NewMetaTracking(sqlDb), inserter_mock{}, inserter_mock{})
